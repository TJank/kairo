...
export default function WeekGrid({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error((json as Record<string, unknown>)?.detail as string || "Request failed");
      }
    ...
    for (const d of days) {
      ...
      const dayItems = entries
        ...
        .map((e) => {
          ...
          return { ...e, dayKey, startSlot, endSlot, col: 0, cols: 1 } as Positioned;
        })
            ...
            {days.map((d) => (
              <div
                ...
                className={`sticky top-0 z-20 bg-zinc-950/90 p-3 text-sm font-semibold ring-1 ring-white/10 ${
                  onDayClick ? "cursor-pointer hover:bg-white/5 transition-colors" : ""
                }`}
                onClick={() => onDayClick?.(d)}
                title={onDayClick ? `Switch to day view for ${format(d, "EEE MMM d")}` : undefined}
              >
              ...
              {days.map((d) => {
                ...
                return (
                  <div key={dayKey} className="relative bg-black/20 ring-1 ring-white/10">
                    ...
                    <div className="absolute inset-0" style={{ height }} aria-hidden={false}>
                      {slots.map((slot) => (
                        <div
                          key={slot}
                          ...
                          style={{ height: SLOT_HEIGHT }}
                          onClick={() =>
                            setAddModal({ date: d, startSlotMin: slotToMins(slot) })
                          }
